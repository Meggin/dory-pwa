...
function loadAndDisplayAnswers(questionId) {
  var myUserId = firebase.auth().currentUser.uid;
  var viewAnswersRef = firebase.database().ref('question-answers/' + questionId);
  ...
  fetchAnswers(viewAnswersRef, questionId, questionCommentsSection);

  listeningFirebaseRefs.push(viewAnswersRef);
    listeningFirebaseRefs.forEach(function(ref) {
    ref.off();
  });
  //viewAnswersRef.off('child_added');
...
function cleanupUi() {
  ...
  //questionCommentsSection.getElementsByClassName('answers-container')[0].innerHTML = '';
